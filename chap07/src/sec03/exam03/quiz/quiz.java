...
//	
//  직원의 정보를 출력하는 getInfo() 메소드도 정규직, 비정규직에 따라 다른 정보를 출력하므로 추상 메소드로 구현합니다.
//	정규직은 직원 구분(type), 이름(name), 소속 부서(dept), 직원번호(empNo), 연봉(salary)을 출력하고
//	비정규직은 직원 구분(type), 이름(name), 소속 부서(dept), 기본임금(primaryPay)을 출력하도록 작성합니다.
//
