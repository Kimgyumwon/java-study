...
//		OutputStream os = new FileOutputStream("C:/Temp/test.db"); // 바이트를 파일에 출력해서 저장하는 기능을 가짐
...
//		byte c = 67;
		...
		char c = '다';
		// 해결 방법 2가지
		// 1. 문자 기반 FileWiter 사용 또는 (OutputStreamWriter 사용)
		// 2. 문자열을 직접 인코딩을 지정하여 바이트 배열로 변환해서 쓰기
		...
		os.flush(); // 버퍼를 비우는 메소드 (버퍼에 있는 데이터들을 한번에 출력)
		
		// 더 이상 쓰지 않을 때는 출력 스트림 닫기
		os.close(); // 출력 스트림이 사용했던 자원들(메모리)을 풀어주기 위해
		// 참고: close() 안에서도 스트림을 닫기 전에 flush() 기능을 수행 후 닫음
