...
//		return age - other.age;
		...
		//나이 내림차순
		// 오름차순 결과의 반대로 구현. 즉, other의 값이 클 때 자리를 바꿈
...
//		return 0;
		...
		// 문자열은 연산자로 비교 불가능
		// String 클래스에 재정의된 comparTo() 사용
		// 기준 문자열이 사전상 뒤면 자리 바꿈
		// 기준 문자열이 사전상 앞이면 자리 바꿈
//		"김성옆".compareTo(other.name); -> 음수 -> 그대로 자리 유지
//		return name.compareTo(other.name); // 이름 오름차순
		return other.name.compareTo(name); // 이름 내림차순 
